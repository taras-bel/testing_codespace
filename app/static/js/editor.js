document.addEventListener('DOMContentLoaded', () => { 
    ...
    CodeMirror.modeURL = 
"https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.15/mode/%N/%N.
 min.js"; 
    ...
    document.addEventListener('mousemove', e => { 
        ...
        const newEditorHeightPercent = (newEditorHeight / totalHeight) 
* 100; 
 
        if (newEditorHeightPercent > 20 && newEditorHeightPercent < 
80) { 
            editorPane.style.height = `${newEditorHeightPercent}%`; 
            bottomPane.style.height = `${100 - 
newEditorHeightPercent}%`; 
        } 
    }); 
    ...
    langSelect.addEventListener('change', (e) => { 
        ...
        setLanguage(newLang); 
        socket.emit('language_change', { session_id: SESSION_ID, 
language: newLang }); 
    }); 
    ...
    const sendChatMessage = () => { 
        ...
        if (message) { 
            socket.emit('chat_message', { session_id: SESSION_ID, 
message }); 
            chatInput.value = ''; 
        } 
    }; 
    ...
    function addChatMessage(user, message) { 
        ...
        msgEl.innerHTML = `<strong>${escapeHtml(user)}:</strong> 
${escapeHtml(message)}`; 
    ...
    editor.on('change', () => { 
        if (!state.isUpdateFromSocket) { 
            socket.emit('code_change', { session_id: SESSION_ID, code: 
editor.getValue() }); 
        } 
    }); 
 
    editor.on('cursorActivity', () => { 
        socket.emit('cursor_move', { session_id: SESSION_ID, position: 
editor.getCursor() }); 
    }); 
    ...
    socket.on('cursor_update', (data) => { 
        ...
        state.remoteCursors[sid] = editor.addWidget(position, 
cursorEl, false); 
    }); 
