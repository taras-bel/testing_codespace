...
from app.execution.runner import execute_code_in_docker, 
AVAILABLE_LANGUAGES 
...
def register_socket_events(socketio): 
    ...
    def on_join(data): 
        ...
        # Уведомляем всех о новом участнике и отправляем обновленный 
список 
        emit('update_participants', {'users': 
list(rooms[session_id].values())}, room=session_id) 
        emit('user_activity', {'message': f'Пользователь 
{current_user.username} присоединился.'}, room=session_id) 
    ...
    def on_disconnect(): 
        ...
        for session_id, participants in rooms.items(): 
            if request.sid in participants: 
                ...
                emit('update_participants', {'users': 
list(participants.values())}, room=session_id) 
                emit('user_activity', {'message': f'Пользователь 
{username} отключился.'}, room=session_id) 
                break 
    ...
    def on_code_change(data): 
        ...
        if session: 
            ...
            db.session.commit() 
            emit('code_update', {'code': session.code}, 
room=session_id, skip_sid=request.sid) 
    ...
    def on_language_change(data): 
        ...
        if session and new_language in AVAILABLE_LANGUAGES: 
            ...
            db.session.commit() 
            emit('language_update', {'language': new_language}, 
room=session_id) 
    ...
    def on_execute_code(data): 
        ...
        def run_execution(app): 
            with app.app_context(): 
                ...
                db.session.commit() 
                socketio.emit('execution_result', {'output': 
final_output}, room=session_id) 
