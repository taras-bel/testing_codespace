...
AVAILABLE_LANGUAGES = {
    'python': {'name': 'Python', 'extension': 'py', 'image': 'python:3.9-slim'},
    'javascript': {'name': 'JavaScript', 'extension': 'js', 'image': 'node:16-slim'},
    'cpp': {'name': 'C++', 'extension': 'cpp', 'image': 'gcc:latest'},
    'java': {'name': 'Java', 'extension': 'java', 'image': 'openjdk:11-jre-slim'},
    'go': {'name': 'Go', 'extension': 'go', 'image': 'golang:1.17-alpine'},
    'rust': {'name': 'Rust', 'extension': 'rs', 'image': 'rust:latest'},
    'csharp': {'name': 'C#', 'extension': 'cs', 'image': 'mcr.microsoft.com/dotnet/sdk:6.0'},
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            command = [
                ...
                lang_config['image'],
                'timeout', str(timeout),
                *get_run_command(language, f'main.{lang_config["extension"]}')
            ]
            ...
            if result.returncode == 124: # Код возврата `timeout`
                 return {'error': f'Выполнение кода превысило лимит времени ({timeout} секунд).'}

            return {'output': result.stdout, 'error': result.stderr}

    except FileNotFoundError:
        return {'error': 'Docker не найден. Для безопасного выполнения кода необходим установленный Docker.'}
    except subprocess.TimeoutExpired:
        return {'error': f'Выполнение кода превысило общий лимит времени.'}
    except Exception as e:
        return {'error': f'Произошла внутренняя ошибка сервера: {str(e)}'}

def get_run_command(language, filename):
    """Возвращает команду для запуска файла в зависимости от языка."""
    if language == 'python':
